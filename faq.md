...
- **Database query rewriting** for R2DBC (SQL) and MongoDB in Spring, with equivalents in the Python, NestJS, and PHP SDKs. Policies attach obligations that add WHERE clauses or filter expressions to database queries at the query level, not post-retrieval. Row-level security enforced before data leaves the database.
...
- **Streaming enforcement** across all SDKs with a single annotation. DENY terminates the stream, SUSPEND pauses it and the next PERMIT resumes it on the same connection, and an optional flag surfaces every suspend and resume boundary to the application.
